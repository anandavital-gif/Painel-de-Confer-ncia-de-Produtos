...
export default function App() {
  ...
  const handleSearch = useCallback(async (searchQuery) => {
    ...
    try {
      ...
      if (list.length === 0) {
        setError('Nenhum NCM encontrado para a busca informada.')
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-4 py-8 space-y-6">
        ...
        {!loading && !error && selected && (
          <NCMDetail ncm={selected} onBack={results.length > 1 ? handleBack : null} />
        )}
      </main>
      ...
      <footer className="border-t border-gray-200 bg-white py-4 mt-8">
        <p className="text-center text-xs text-gray-400">
          ...
          · Legislação tributária: Receita Federal do Brasil / CONFAZ
        </p>
      </footer>
    </div>
  )
